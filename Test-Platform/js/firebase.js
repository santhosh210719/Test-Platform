...
const LOCAL_SCHEDULED_TESTS_KEY = 'tp_scheduled_tests_local_v1';

function readLocalScheduledTests() {
    try {
        var raw = localStorage.getItem(LOCAL_SCHEDULED_TESTS_KEY);
        if (!raw) return [];
...
function writeLocalScheduledTests(tests) {
    try {
        localStorage.setItem(LOCAL_SCHEDULED_TESTS_KEY, JSON.stringify(tests));
    } catch (e) {
        console.warn('[Tests] local cache write failed:', e);
    }
}
...
/**
 * Save a new test to Firestore, or to localStorage if Firestore is not configured.
 * @param {Object} test - The test object.
 * @returns {Promise<{ docId: string, localOnly: boolean }>}
 */
async function saveTestToFirestore(test) {
    if (!db) {
        ...
        var list = readLocalScheduledTests();
        list.push(localTest);
        writeLocalScheduledTests(list);
        console.warn('[Tests] Saved scheduled test locally (Firebase not configured). Students must join from this same browser.');
        return { docId: 'local_' + String(test.testId), localOnly: true };
    }
    ...
    try {
        ...
        return { docId: docRef.id, localOnly: false };
    } catch (e) {
...
async function getTestById(testId) {
    ...
    for (var i = list.length - 1; i >= 0; i--) {
        if (String(list[i].testId) === id) return list[i];
    }
    return null;
}
...
async function testIdExists(testId) {
    ...
    for (var i = 0; i < list.length; i++) {
        if (String(list[i].testId) === String(testId)) return true;
    }
    return false;
}
