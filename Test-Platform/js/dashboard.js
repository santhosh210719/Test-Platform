  ...
  if (scheduleForm) {
      scheduleForm.addEventListener('submit', async (e) => {
          ...
          try {
              ...
              if (result.localOnly) {
                baseMsg +=
                  ' Note: Firebase is off — this test is stored in this browser only; students must use this same device/profile to join, or configure firebase-config.local.js.';
              }
              showTemporarySuccess(baseMsg);
          } catch (err) {
              console.error(err);
