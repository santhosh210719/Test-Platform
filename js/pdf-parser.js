...
(function () {
  ...
  async function extractMcqsWithGemini(text) {
    ...
    var prompt =
      ...
      'TEXT:\n' + text.slice(0, 12000); // limit tokens
    ...
    if (!res.ok) {
      ...
      var errMessage = 'Gemini API error: ' + res.status;
      try {
      ...
      if (errMessage.includes('leaked')) {
        throw new Error('Your Gemini API Key has been blocked by Google because it was leaked online. Set a new key in js/firebase-config.local.js');
      } else if (errMessage.includes('API key not valid')) {
        throw new Error('Invalid Gemini API Key. Check js/firebase-config.local.js or localStorage "geminiApiKey".');
      } else {
