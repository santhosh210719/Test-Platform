...
/**
 * Google AI Studio / Gemini API key only (no Groq fallback).
 * Use this to decide whether to call generativelanguage.googleapis.com.
 */
function getGoogleGeminiApiKeyOnly() {
  if (typeof window !== "undefined" && window.GEMINI_API_KEY_OVERRIDE) {
    var go = String(window.GEMINI_API_KEY_OVERRIDE).trim();
    if (go) return go;
  }
